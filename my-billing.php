...
$driveActive = 'billing';
$driveCrumbs = [['label' => 'Home', 'href' => '/index.php'], ['label' => 'My billing']];
$driveTitle = 'My billing';
$driveSubtitle = 'Pick a plan, pay using GCash, PayPal, or card, then upload your proof of payment. Methods marked <strong>⚡ Instant activation</strong> unlock your plan the moment you submit — everything else is reviewed by an admin, usually within a day.';
  ...
  <div class="billing-page">
    ...
    <section class="admin-card glass-card">
      ...
      <?php if ($currentPlan): ?>
        <p class="admin-sub">
          ...
          — <?php echo inkwell_marketplace_can_sell($me) ? 'marketplace selling unlocked ✅' : 'marketplace selling locked 🔒'; ?>
        </p>
      <?php else: ?>
    ...
    <section class="pricing-grid" id="plans">
      <?php foreach ($plans as $plan): $features = inkwell_plan_features($plan);
        ...
        $unlocksSelling = inkwell_plan_unlocks_marketplace_selling($plan);
      ?>
        <div class="plan-card glass-card<?php echo !empty($plan['badge']) ? ' featured' : ''; ?>">
          ...
          <?php else: ?>
            ...
            <?php if ($unlocksSelling): ?><p class="plan-locked-note" style="color:var(--pine);border-style:solid;">Unlocks marketplace selling — buy &amp; sell your own systems ✅</p><?php endif; ?>
          <?php endif; ?>
          <?php if ($features): ?>
