        ...
        postAjax({ action: 'delete_post', post_id: postId })
          .then(function (data) {
            ...
            if (feed && !feed.querySelector('.post-card')) {
              feed.insertAdjacentHTML('afterbegin', '<div class="profile-empty-tab" id="postFeedEmpty"><span class="icon">🖼️</span><p class="admin-sub" style="margin:0;">You haven\'t posted anything yet. <a href="/posts.php">Share something →</a></p></div>');
            }
          })
          .catch(function () { alert('Network error — please try again.'); })
