...
include __DIR__ . '/includes/post_author_profile_modal.php';
?>
        ...
        postAjax({ action: 'delete_post', post_id: postId })
          .then(function (data) {
            ...
            if (feed && !feed.querySelector('.post-card')) {
              feed.insertAdjacentHTML('afterbegin', '<div class="admin-card glass-card post-empty" id="postFeedEmpty"><span class="icon">🖼️</span><p class="admin-sub" style="margin:0;">No posts yet — be the first to share something.</p></div>');
            }
          })
          .catch(function () { alert('Network error — please try again.'); });
        return;
