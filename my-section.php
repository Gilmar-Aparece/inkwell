...
// Teachers: browse by section — each section card lists the subjects tagged
// to it, and each subject links through to its exams. Subjects a teacher
// hasn't tagged to a section yet still show up in a trailing group below.
if ($user['role'] === 'teacher') {
  $__me = $user;
  $mySections = inkwell_teacher_all_sections($user['id']);
  $__allMySubjects = inkwell_teacher_subjects($user['id']);
  $__ungroupedSubjects = array_values(array_filter($__allMySubjects, function ($s) {
    return empty($s['section_id']);
  }));
  ...
  $driveSubtitle = 'Your sections — open one to see its subjects, then a subject to see its exams.';
  include __DIR__ . '/includes/drive_shell_top.php';
  ?>
    <?php if (empty($mySections) && empty($__ungroupedSubjects)): ?>
      <section class="admin-card glass-card">
        <h2>No sections yet</h2>
        <p class="admin-sub">Create a section from <a href="/teacher/subject.php">Subjects</a> to see it here.</p>
      </section>
    <?php else: ?>
        ...
        <section class="admin-card glass-card">
          <div class="admin-header-row" style="margin-bottom:0;">
            <div>
              ...
              <p class="admin-sub"><?php echo (int) $sec['subject_count']; ?> subject<?php echo (int) $sec['subject_count'] === 1 ? '' : 's'; ?><?php echo !empty($sec['term']) ? ' · ' . htmlspecialchars($sec['term']) : ''; ?><?php echo !empty($sec['academic_year']) ? ' ' . htmlspecialchars($sec['academic_year']) : ''; ?><?php echo !empty($sec['year_level']) ? ' · ' . htmlspecialchars($sec['year_level']) : ''; ?></p>
            </div>
          </div>
          ...
          <?php else: ?>
            <div class="drive-grid">
              <?php foreach ($subjects as $subj): ?>
                <a class="drive-file-card" href="/exams.php?subject=<?php echo (int) $subj['id']; ?>">
      ...
      <?php if (!empty($__ungroupedSubjects)): ?>
        <section class="admin-card glass-card">
          <div class="admin-header-row" style="margin-bottom:0;">
            <div>
              <h2>Not yet in a section</h2>
              <p class="admin-sub">Tag these to a section from <a href="/teacher/subject.php">Subjects</a> to have them show up above.</p>
            </div>
          </div>
          <div class="drive-grid">
            <?php foreach ($__ungroupedSubjects as $subj): ?>
              <a class="drive-file-card" href="/exams.php?subject=<?php echo (int) $subj['id']; ?>">
                <div class="drive-file-cover" style="background:linear-gradient(135deg, #6c5ce726, #6c5ce70d);">
                  <span class="drive-file-icon" style="background:#6c5ce71f; color:#6c5ce7;">📚</span>
                  <?php if (!empty($subj['code'])): ?><span class="drive-file-badge"><?php echo htmlspecialchars($subj['code']); ?></span><?php endif; ?>
                ...
                <div class="drive-file-body">
                  <span class="drive-file-name"><?php echo htmlspecialchars($subj['title']); ?></span>
                  <div class="drive-file-meta">
                    <span><?php echo (int) $subj['units']; ?> unit<?php echo (int) $subj['units'] === 1 ? '' : 's'; ?></span>
                    <?php if (!empty($subj['exam_count'])): ?><span><?php echo (int) $subj['exam_count']; ?> exam<?php echo (int) $subj['exam_count'] === 1 ? '' : 's'; ?></span><?php endif; ?>
                  </div>
                </div>
              </a>
            <?php endforeach; ?>
