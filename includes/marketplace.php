...
function inkwell_marketplace_thumb_html($listing, $iconFallback = '💻') {
  if (inkwell_marketplace_has_embeddable_preview($listing)) {
    ...
    return '<div class="mkt-live-embed" data-mkt-live-embed data-fallback-icon="' . htmlspecialchars($iconFallback) . '">'
...
function inkwell_marketplace_redeem_code($listingId, $buyer, $rawCode) {
  ...
  // Let the seller know right away that someone bought in — covers codes
  // handed out directly (no purchase request), which otherwise generated
  ...
  if ($listing) {
    inkwell_create_notification(
      $listing['seller_id'],
      'marketplace_sale',
      substr('💰 ' . $buyer['name'] . ' just unlocked "' . $listing['title'] . '"', 0, 250),
      '/sell-transactions.php'
    );
  }
