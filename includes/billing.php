...
require_once __DIR__ . '/notifications.php'; // every transaction below notifies the user (and admins, where relevant)
...
function inkwell_ensure_billing_columns() {
  ...
  $planCols = [
    ...
    // Lets a plan double as a "marketplace seller" upgrade — any account
    // (not just students/teachers) that's on an active plan with this flag
    // on can list systems for sale on /sell.php. See
    // inkwell_user_has_marketplace_selling_access() and
    // inkwell_marketplace_can_sell() in includes/marketplace.php. Defaults
    // to OFF for every existing plan (including paid ones) so nothing
    // changes for current subscribers until an admin opts a plan in.
    'unlocks_marketplace_selling' => "ALTER TABLE plans ADD COLUMN unlocks_marketplace_selling TINYINT(1) NOT NULL DEFAULT 0",
  ];
  try {
...
/** Whether being on this plan (once active) lets the account sell on the marketplace, regardless of role. */
function inkwell_plan_unlocks_marketplace_selling($plan) {
  if (!$plan) return false;
  return !empty($plan['unlocks_marketplace_selling']);
}
...
function inkwell_create_plan($data) {
  ...
  $unlocksSelling = !empty($data['unlocks_marketplace_selling']) ? 1 : 0;
  ...
  $pdo = inkwell_db();
  $stmt = $pdo->prepare('INSERT INTO plans (name, audience, price, price_yearly, billing_period, description, features, badge, unlocks_exams, unlocks_all_lessons, unlocks_marketplace_selling, is_active, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)');
  $stmt->execute([$name, $audience, $price, $priceYearly, $period, $description ?: null, $features ?: null, $badge ?: null, $unlocksExams, $unlocksAllLessons, $unlocksSelling, $sort]);
...
function inkwell_update_plan($id, $data) {
  ...
  $pdo = inkwell_db();
  $stmt = $pdo->prepare('UPDATE plans SET name=?, audience=?, price=?, price_yearly=?, billing_period=?, description=?, features=?, badge=?, unlocks_exams=?, unlocks_all_lessons=?, unlocks_marketplace_selling=?, sort_order=? WHERE id=?');
  $stmt->execute([$name, $audience, $price, $priceYearly, $period, $description ?: null, $features ?: null, $badge ?: null, $unlocksExams, $unlocksAllLessons, $unlocksSelling, $sort, (int) $id]);
  return ['ok' => true];
}
...
function inkwell_submit_payment($userId, $planId, $methodId, $referenceNo, $proofFileField = 'proof_image', $cycle = 'month', $senderNumber = '', $paymentDate = '') {
  ...
  if ($amount <= 0) {
    ...
    inkwell_create_notification($userId, 'billing', 'Your "' . $plan['name'] . '" plan is now active — enjoy!', '/my-billing.php');
  ...
  if ($instant) {
    ...
    inkwell_create_notification($userId, 'billing', '🎉 Payment received — your "' . $plan['name'] . '" plan is active now.', '/my-billing.php');
    $__notifUser = inkwell_get_user($userId);
    inkwell_notify_admins('billing', ($__notifUser ? $__notifUser['name'] : 'A user') . ' paid ₱' . number_format($amount, 2) . ' for "' . $plan['name'] . '" (instant activation).', '/admin/payments.php?status=approved');
  ...
  inkwell_create_notification($userId, 'billing', 'Payment submitted for "' . $plan['name'] . '" — pending review. We\'ll activate your plan once an admin approves it.', '/my-billing.php');
  $__notifUser = inkwell_get_user($userId);
  inkwell_notify_admins('billing', 'New payment submission from ' . ($__notifUser ? $__notifUser['name'] : 'a user') . ' — ₱' . number_format($amount, 2) . ' for "' . $plan['name'] . '".', '/admin/payments.php?status=pending');
...
function inkwell_create_school_checkout($schoolName, $name, $email, $password, $idNumber, $course, $planId, $methodId, $referenceNo, $proofFileField = 'proof_image', $cycle = 'month', $senderNumber = '', $paymentDate = '') {
  ...
  $plan = inkwell_get_plan($planId);
  if (!$plan || !in_array($plan['audience'], ['school', 'both'], true) || inkwell_plan_unlocks_marketplace_selling($plan)) {
...
function inkwell_review_payment_submission($id, $approve, $adminNote = '') {
  ...
  $plan = inkwell_get_plan($sub['plan_id']);
  ...
  if ($approve) {
    ...
    inkwell_create_notification($sub['user_id'], 'billing', '✅ Your payment for "' . $planName . '" was approved — your plan is active.', '/my-billing.php');
  } else {
    $stmt2 = $pdo->prepare('UPDATE users SET plan_status = \'expired\' WHERE id = ? AND plan_id = ?');
    $stmt2->execute([(int) $sub['user_id'], (int) $sub['plan_id']]);
    $__rejectMsg = '❌ Your payment for "' . $planName . '" was rejected.' . (trim($adminNote) !== '' ? ' Note: ' . trim($adminNote) : '');
    inkwell_create_notification($sub['user_id'], 'billing', $__rejectMsg, '/my-billing.php');
  }
...
/**
 * True if this user is allowed to sell on the marketplace purely because of
 * their plan (as opposed to their role — see inkwell_marketplace_can_sell()
 * in includes/marketplace.php, which ORs this together with the built-in
 * student/teacher allowance). Mirrors inkwell_user_has_exam_access(): reads
 * straight off plan_id/plan_status, so BOTH activation paths in this file
 * (the instant auto-activate branch and the admin-approve branch, which
 * both call inkwell_activate_user_plan()) grant this the moment the plan
 * goes active — nothing extra to flip on the user row itself.
 */
function inkwell_user_has_marketplace_selling_access($user) {
  if (!$user) return false;
  if (($user['plan_status'] ?? 'none') !== 'active') return false;
  if (!empty($user['plan_expires_at']) && strtotime($user['plan_expires_at']) < time()) return false;
  $plan = !empty($user['plan_id']) ? inkwell_get_plan($user['plan_id']) : null;
  return inkwell_plan_unlocks_marketplace_selling($plan);
}
