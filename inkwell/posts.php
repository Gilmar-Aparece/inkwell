...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'report_post') {
    ...
    if ($ajax) inkwell_json_response($result);
    inkwell_flash_set($result['ok'] ? 'notice' : 'error', $result['ok'] ? 'Thanks — this post has been reported.' : $result['error']);
    header('Location: /posts.php');
    exit;
  }

  if ($action === 'edit_post') {
    $result = inkwell_edit_post($postId, $user['id'], $_POST['caption'] ?? '', $_POST['text_align'] ?? 'left', !empty($_POST['text_bold']), $isAdmin);
    if ($result['ok']) {
      $post = inkwell_get_post_full($postId, $user['id']);
      if ($ajax) inkwell_json_response(['ok' => true, 'caption' => $post['caption'], 'html' => inkwell_render_post_card($post, $user, $isAdmin)]);
