...
/** True once the `locked_at` column exists on `users` (cached per request) — used by account lock/unlock. */
function inkwell_users_lock_column_ok() {
  static $ok = null;
  if ($ok !== null) return $ok;
  try {
    inkwell_db()->query('SELECT locked_at FROM users LIMIT 1');
    $ok = true;
...
/** Self-healing: adds the `locked_at` column on first use if this host allows DDL over a normal DB connection. */
function inkwell_ensure_users_lock_column() {
  static $checked = false;
  if ($checked) return;
  $checked = true;
  if (!inkwell_users_lock_column_ok()) {
    try {
      inkwell_db()->exec('ALTER TABLE users ADD COLUMN locked_at datetime DEFAULT NULL');
    } catch (PDOException $e) {
      // Blocked or already applied — inkwell_users_lock_column_ok() keeps checking.
    }
  }
}
...
function inkwell_login_user($identifier, $password) {
  ...
  inkwell_ensure_users_lock_column();
  if (inkwell_users_lock_column_ok() && !empty($user['locked_at'])) {
    return ['ok' => false, 'locked' => true, 'error' => 'This account is locked.'];
  }
...
/**
 * Self-lock: the account holder confirms their current password, then
 * their account is marked locked and they're immediately logged out.
 * There's no email/reset infrastructure in this app, so proving you still
 * know the password (via /unlock-account.php) is the unlock mechanism —
 * see inkwell_unlock_account_with_password() below.
 */
function inkwell_lock_account($userId, $password) {
  $pdo = inkwell_db();
  ...
  inkwell_ensure_users_lock_column();
  if (!inkwell_users_lock_column_ok()) {
    return ['ok' => false, 'error' => "Couldn't lock the account right now — try again in a moment."];
  }
  $stmt = $pdo->prepare('UPDATE users SET locked_at = NOW() WHERE id = ?');
  $stmt->execute([$userId]);
  inkwell_logout_user();
  return ['ok' => true];
}
...
/**
 * Unlocks a locked account by re-confirming the identifier + password
 * (same credentials as login) — proves the person asking to unlock it is
 * still its owner — then logs them straight in.
 */
function inkwell_unlock_account_with_password($identifier, $password) {
  $identifier = trim($identifier);
  $pdo = inkwell_db();
  ...
  inkwell_ensure_users_lock_column();
  if (!inkwell_users_lock_column_ok()) {
    return ['ok' => false, 'error' => "Couldn't unlock the account right now — try again in a moment."];
  }
  $upd = $pdo->prepare('UPDATE users SET locked_at = NULL WHERE id = ?');
  $upd->execute([$user['id']]);
...
/** True if this user's account is currently locked. */
function inkwell_account_is_locked($userId) {
  inkwell_ensure_users_lock_column();
  if (!inkwell_users_lock_column_ok()) return false;
  $pdo = inkwell_db();
  $stmt = $pdo->prepare('SELECT locked_at FROM users WHERE id = ?');
  $stmt->execute([$userId]);
