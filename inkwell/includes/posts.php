...
function inkwell_ensure_posts_tables() {
  ...
  // Facebook-style post menu (Hide / Report / Edit) — hidden_posts and
...
/**
 * Simple engagement analytics for the "Analytics" tab on a user's own
 * profile: totals across all of their posts, plus their 5 best-performing
 * posts by (likes + comments + saves). Cheap aggregate queries, no new
 * tables required.
 */
function inkwell_user_engagement_stats($userId) {
  inkwell_ensure_posts_tables();
  $out = [
    'post_count' => 0, 'likes_received' => 0, 'comments_received' => 0,
    'saves_received' => 0, 'top_posts' => [],
  ];
  try {
    $pdo = inkwell_db();
    $stmt = $pdo->prepare('SELECT COUNT(*) FROM posts WHERE user_id = ?');
    $stmt->execute([$userId]);
    $out['post_count'] = (int) $stmt->fetchColumn();

    $stmt = $pdo->prepare('SELECT COUNT(*) FROM post_likes pl JOIN posts p ON p.id = pl.post_id WHERE p.user_id = ?');
    $stmt->execute([$userId]);
    $out['likes_received'] = (int) $stmt->fetchColumn();

    $stmt = $pdo->prepare('SELECT COUNT(*) FROM post_comments pc JOIN posts p ON p.id = pc.post_id WHERE p.user_id = ?');
    $stmt->execute([$userId]);
    $out['comments_received'] = (int) $stmt->fetchColumn();

    if (inkwell_posts_saves_table_ok()) {
      $stmt = $pdo->prepare('SELECT COUNT(*) FROM post_saves ps JOIN posts p ON p.id = ps.post_id WHERE p.user_id = ?');
      $stmt->execute([$userId]);
      $out['saves_received'] = (int) $stmt->fetchColumn();
    }
    ...
    usort($rows, function ($a, $b) {
      $scoreA = (int) $a['like_count'] + (int) $a['comment_count'] + (int) $a['save_count'];
      $scoreB = (int) $b['like_count'] + (int) $b['comment_count'] + (int) $b['save_count'];
      return $scoreB <=> $scoreA;
    });
    $out['top_posts'] = array_slice($rows, 0, 5);
  } catch (PDOException $e) {
    // Leave defaults — analytics is a nice-to-have, never worth a hard error.
  }
  return $out;
}
...
function inkwell_edit_post($postId, $userId, $caption, $textAlign = 'left', $textBold = false, $isAdmin = false) {
  $post = inkwell_get_post($postId);
  if (!$post) return ['ok' => false, 'error' => 'Post not found.'];
  if (!$isAdmin && (int) $post['user_id'] !== (int) $userId) {
  ...
  try {
    $pdo = inkwell_db();
    if (inkwell_posts_style_columns_ok()) {
      $stmt = $pdo->prepare('UPDATE posts SET caption = ?, text_align = ?, text_bold = ? WHERE id = ?');
      $stmt->execute([$caption !== '' ? $caption : null, $textAlign, $textBold ? 1 : 0, $postId]);
    } else {
...
function inkwell_render_post_card($post, $user, $isAdmin) {
  ...
  <article class="admin-card glass-card post-card" id="post-<?php echo (int) $post['id']; ?>">
      ...
      <div class="post-menu-wrap" data-post-menu>
        ...
        <div class="post-menu-dropdown" data-post-menu-dropdown hidden>
          <?php if ($canDeletePost): ?>
            <?php if ((int) $post['user_id'] === (int) $user['id'] || $isAdmin): ?>
              <button type="button" class="post-menu-item" data-post-edit data-post-id="<?php echo (int) $post['id']; ?>">
          ...
          <?php else: ?>
            ...
            <button type="button" class="post-menu-item danger" data-post-report data-post-id="<?php echo (int) $post['id']; ?>">
    ...
    <?php if ((int) $post['user_id'] === (int) $user['id'] || $isAdmin): ?>
      <form class="post-edit-form" data-post-edit-form data-post-id="<?php echo (int) $post['id']; ?>" hidden>
        <textarea name="caption" maxlength="5000" placeholder="What's on your mind?"><?php echo htmlspecialchars($post['caption'] ?? ''); ?></textarea>
        <div class="post-edit-form-actions">
          <button type="button" class="btn" data-post-edit-cancel>Cancel</button>
