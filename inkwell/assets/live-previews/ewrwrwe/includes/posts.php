...
function inkwell_list_new_posts($afterId, $viewerId, $limit = 20) {
  ...
  try {
    ...
    $stmt->execute([$viewerId, $viewerId, (int) $afterId]);
    $rows = $stmt->fetchAll();
    return inkwell_attach_gallery_images($rows);
  } catch (PDOException $e) {
    return [];
  ...
  <article class="admin-card glass-card post-card" id="post-<?php echo (int) $post['id']; ?>">
    <div class="post-head">
      ...
      <?php if ($canDeletePost): ?>
        <button type="button" class="post-kebab-btn danger-hover" data-post-delete data-post-id="<?php echo (int) $post['id']; ?>" title="Delete post" aria-label="Delete post">⋯</button>
      <?php endif; ?>
    </div>
    ...
    <?php if (!empty($post['caption'])): ?>
      <?php if ($capBgId): ?>
        <p class="post-caption text-only post-caption-bg post-tpl-<?php echo htmlspecialchars($capBgId); ?>" style="background:<?php echo $bgTemplates[$capBgId]; ?>; text-align:<?php echo $capAlign; ?>; justify-content:<?php echo $capJustify; ?>;"><?php echo htmlspecialchars($post['caption']); ?></p>
      <?php elseif ($isTextOnly): ?>
        <p class="post-caption text-only" style="text-align:<?php echo $capAlign; ?>;<?php echo $capBold ? ' font-weight:800;' : ''; ?>"><?php echo htmlspecialchars($post['caption']); ?></p>
      <?php else: ?>
