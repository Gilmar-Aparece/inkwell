...
$viewer = inkwell_current_user(); // optional — this popup is intentionally public for guests too
...
<?php if ($viewer && (int) $t['id'] !== (int) $viewer['id']): ?>
  <a class="btn primary" href="/messages.php?with=<?php echo (int) $t['id']; ?>" style="display:block;text-align:center;margin:0 0 16px;">💬 Message</a>
<?php endif; ?>
      ...
      <div class="student-profile-list">
        <?php foreach ($profile['teachers'] as $teach): ?>
          <div class="student-profile-list-row">
            ...
            <?php if (inkwell_can_view_email($teach, $viewer)): ?>
              <strong><?php echo htmlspecialchars($teach['email']); ?></strong>
            <?php else: ?>
              <strong class="admin-sub" style="font-weight:600;">Email private</strong>
            <?php endif; ?>
          </div>
        <?php endforeach; ?>
      </div>
